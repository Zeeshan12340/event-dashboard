...
export default function EventCard() {
  ...
  // Feature the first loaded event; hide the card when there is nothing to show.
  const featured = events.find((event: Event) => event.title);

  if (page.expandEvents || !featured) {
    return null;
  }

  const location = featured.entities[0]?.name ?? featured.timezone;
      ...
      <div className="bg-white rounded-lg w-full p-3">
        <div className="flex w-full">
          <div className="w-full">
            <div className="font-bold text-blue-800">{featured.title}</div>
          ...
          <div className="flex flex-wrap justify-end">
            ...
            <div className="w-full text-gray-400 text-end">
              {convertToLocalTime(featured.start, true, featured.timezone)}
            </div>
            <div className="w-full text-gray-400 text-end">
              {convertToLocalTime(featured.start, false, featured.timezone)}
            </div>
          </div>
        </div>
      </div>
